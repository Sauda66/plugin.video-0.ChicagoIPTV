...
addon = xbmcaddon.Addon('plugin.video.ChicagoIPTV')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.ChicagoIPTV-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(ChicagoIPTV,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(ChicagoIPTV,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(ChicagoIPTV,New source added.,5000,"+icon+")")
        if not url is None:
...
#hakamac code
    ...
    if keyboard.isConfirmed():
       ...
       if len(Searchkey) == 0: 
          xbmcgui.Dialog().ok('ChicagoIPTV', 'Nothing Entered')
          return	   
    ...
    progress.create('ChicagoIPTV Searching Please wait',' ')
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if not name in FAV:
                contextMenu.append(('Add to ChicagoIPTV Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
